  ...
  const renderArchitectureView = () => (
        <div className="card p-6">
          {archData?.diagram ? (
            <>
              <div className="flex items-center justify-between mb-4 gap-3">
                <div className="text-xs text-text-muted">
                  Layered view of the codebase — subgraphs are layers, edges are calls or data flow.
                </div>
              ...
              {/* Color legend — the backend prompt asks Claude to apply
                  classDef colors to external services, data stores, and
                  workers. Most users won't inspect the source, so surface
                  the meaning here. */}
              <div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 text-xs text-text-muted">
                <span className="inline-flex items-center gap-1.5">
                  <span className="w-3 h-3 rounded border" style={{ background: "#1a2440", borderColor: "#4a90e2" }} aria-hidden="true" />
                  External service
                </span>
                <span className="inline-flex items-center gap-1.5">
                  <span className="w-3 h-3 rounded border" style={{ background: "#1f2a1f", borderColor: "#5a8a5a" }} aria-hidden="true" />
                  Data store
                </span>
                <span className="inline-flex items-center gap-1.5">
                  <span className="w-3 h-3 rounded border" style={{ background: "#2a1f2a", borderColor: "#9a5a9a" }} aria-hidden="true" />
                  Background worker
                </span>
                <span className="inline-flex items-center gap-1.5">
                  <span className="w-3 h-3 rounded border" style={{ background: "#1a1830", borderColor: "#7c6bff" }} aria-hidden="true" />
                  Application code
                </span>
              </div>
            ...
            <EmptyState label="Generate a layered Mermaid diagram of your codebase. Shows components grouped by layer (frontend, API, services, data, external) with the calls and data flow between them." />
          )}
        </div>
    );
...
function MermaidDiagram({ chart }: { chart: string }) {
  ...
  // `zoom` defaults to 0 — meaning "auto, not computed yet". An effect
  // below measures the rendered SVG and replaces this with a smart fit
  // value. Until then the diagram is hidden behind the loading state.
  const [zoom, setZoom] = useState(0);
  const [naturalWidth, setNaturalWidth] = useState(0);
  // Cache the computed fit zoom so the "Fit" button can return to it.
  // Recomputed whenever the container is resized OR a new SVG is set.
    ...
    (async () => {
      try {
        ...
        mermaid.initialize({
          ...
          // Tuned for the layered LR diagrams the backend now generates:
          //  - LR flow, generous spacing so subgraphs read as distinct regions
          //  - htmlLabels off in strict security mode (renders as SVG text — better contrast)
          //  - curve basis softens edge corners without losing precision
          flowchart: {
            // useMaxWidth disabled — we render at natural size and rely on the
            // container's overflow-auto + our smart-fit zoom for sizing.
            useMaxWidth: false,
            htmlLabels: false,
            curve: "basis",
            nodeSpacing: 50,
          ...
          themeVariables: {
            // Match the app's brand palette so the diagram doesn't feel grafted on.
            background: "transparent",
            primaryColor: "#1a1830",                 // node fill — deep violet
            primaryTextColor: "#e6e8ef",
            primaryBorderColor: "#7c6bff",           // accent stroke
            lineColor: "#8491a5",                    // edge color
            secondaryColor: "#10131c",               // alt node fill (used by clusters)
            tertiaryColor: "#0e1017",                // subgraph background
            clusterBkg: "#0e1017",                   // subgraph (layer) fill
            clusterBorder: "#3a4258",                // subgraph border — softer than node borders so layers read as containers, not nodes
            fontFamily: '"Inter", "IBM Plex Sans", system-ui, sans-serif',
            fontSize: "13px",
            edgeLabelBackground: "#0b0f17",          // small chip behind edge labels so they don't melt into other lines
          },
        });
        const id = "mmd-" + Date.now();
        const { svg } = await mermaid.render(id, chart);
        if (!cancelled) {
          // Keep Mermaid's natural width/height — the container handles overflow
          // with scroll. Forcing width="100%" used to shrink large diagrams down
          // to container width and make all the text microscopic.
          //
          // The only adjustment we make: strip the inline `max-width` Mermaid
          // adds to its <svg> style, because that re-applies the same shrinkage
          ...
          // Parse the SVG's natural width so we can pick a smart initial zoom.
          // Mermaid emits something like: <svg ... width="1842" ...>
          const widthMatch = cleaned.match(/<svg[^>]*\bwidth="(\d+(?:\.\d+)?)/);
          const naturalWidth = widthMatch ? parseFloat(widthMatch[1]) : 0;
  ...
  // Compute the right starting zoom whenever the SVG (re-)renders, or the
  // container resizes. Strategy:
  //
  //   1. Aim for the diagram to fill ~95% of the container width when the
  //      diagram is small. That makes nodes use the available horizontal
  //      space instead of clustering in the middle.
  //   2. When the diagram is wider than the container at its natural size,
  //      DO NOT shrink it to fit — text would get tiny. Instead, keep it
  //      at natural size (zoom = 1) and let the user scroll horizontally.
  //   3. Always keep node text at >= ~13px effective size for readability.
  //
  // The `fitZoom` value is what the "Fit" button restores; `zoom` is the
  // currently-applied scale (may differ if the user has zoomed in/out).
  useEffect(() => {
    if (!svg || !naturalWidth || !containerRef.current) return;
    const computeFit = () => {
      const containerWidth = containerRef.current?.clientWidth ?? 0;
      if (!containerWidth) return;
      // Reserve room for the 1.5rem padding inside the content wrapper.
      const usable = containerWidth - 48;
      // Ratio = how much we'd need to zoom to fit width-wise.
      const widthRatio = usable / naturalWidth;
      // If natural-size already overflows, use natural size (zoom 1) and
      // let the user scroll. Don't shrink the diagram to fit — text
      // legibility wins over "see everything at once."
      // If the diagram is much smaller than the container, scale up to
      // fill ~95% of the width, but cap at 1.8x so we don't blow up
      // small diagrams into wall-art.
      const fit = widthRatio >= 1
        ? Math.min(widthRatio * 0.95, 1.8)
        : 1;
      setFitZoom(fit);
      // On first render only (zoom = 0 means "uninitialized"), apply the
      // computed fit immediately. After that the user is in control.
      setZoom((current) => (current === 0 ? fit : current));
    };
    computeFit();

    // Recompute on resize so the diagram adapts when the panel/window changes.
    // ResizeObserver gives us per-container resize, not just window resize —
    // matters because the panel can change width without the window doing so
    // (e.g. user expanding a sidebar).
    const ro = new ResizeObserver(computeFit);
    ro.observe(containerRef.current);
    return () => ro.disconnect();
  }, [svg, naturalWidth, fullscreen]);
  ...
  const Controls = (
    <div className="flex items-center gap-1 bg-surface-overlay border border-surface-border rounded-lg p-1">
      ...
      <button
        onClick={() => setZoom(1)}
        className="px-2 h-7 text-xs text-text-secondary hover:text-text-primary hover:bg-surface-border rounded transition-colors"
        aria-label="100% — natural size"
        title="100% — natural pixel size"
      >
        1:1
      </button>
      <span className="w-px h-4 bg-surface-border mx-1" aria-hidden="true" />
  ...
  const diagramContent = (
    ...
    >
      <div
        ...
        style={{
          // Apply the smart-computed zoom. Until that effect runs (zoom === 0),
          // keep the diagram visible at scale 1 but transparent so we don't
          // see a flash at natural size before the fit-zoom kicks in.
